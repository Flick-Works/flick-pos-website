...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const phonePreviews = document.querySelectorAll('.phone-mockup--analytics, .phone-mockup--home');

  function swapPhonePreviews() {
    if (!heroVisual) return;

    const isSwapped = heroVisual.classList.toggle('is-swapped');
    phonePreviews.forEach(phone => {
      phone.setAttribute('aria-label', isSwapped ? 'Restore phone preview positions' : 'Swap phone previews');
    });
  }

  phonePreviews.forEach(phone => {
    phone.addEventListener('click', swapPhonePreviews);
